...
$form_title="مرحله اول: انتخاب نوع آگهی";
$page_id=80;
access_page1($page_id,$_SESSION["s_user_type_id"]);
$tbl_name="tbl_car_master_info";
if ($_GET["edit"]==1)
{
	$form_title="ویرایش آگهی خودروی از قبل ثبت شده توسط کاربر";
	$edit=1;
...
var valid_fld=new Array("f1","f2","f3","f3_temp","f4","f5","f6","f5","f12");
var test_array=new Array("1","1","1","1","1", "1", "1","3","1");	
...
<div align="center">
<div class="main_form">
<form method="POST" action="admin_user_reg_car_detail.php" name="frm1" id="frm1">
...
<?php }; ?>		
	...
	<input type=hidden value="<?php print time() ?>" id="f11" name="f11">
	<input type=hidden value="250" id="f1" name="f1">
	<?php
		include "jdf.php";
...
<option value='-1'> آگهی دهنده را انتخاب کنید</option>
<?php 
...
if($user_city!="")
{
 ...
 ?>
					...
					<tr>
						<td align="left" width="20%" class="cell_lable">اولویت 
						نمایش :</td>
						<td align="right" class="cell_input" width="30%">
						...
						نمایش <span lang="en-us">&nbsp;</span>:</td>
						<td class="cell_input" align="right" width="30%">
						<select id="f6" name="f6" size="1"  />
						<option <?php if($edit) if ($selected_row["f6"]=="1") print "selected"?> value="1">نمایش داده شود</option>
						<option <?php if($edit) if ($selected_row["f6"]=="0") print "selected"?> value="0">فعلا نمایش داده نشود</option>
						</select>
					</td>
					</tr>
					<tr>
					...
					<tr>
						<td align="center" width="100%"  colspan="4">
						<input type="checkbox" name="f_roul" id="f_roul"> قوانین واچار724 را مطالعه کرده و می پذیرم
						<br>
...
{
	$("#err_roul").hide();
	$("#err_roul").fadeIn(400).html('<font color=#ff0000><b>لطفا ابتدا قوانین واچار724 را مطالعه نموده و توافق خود را با آن اعلام نمایید</b></font>');
	return;
}
...
success: function(data){
		...
		{
			//alert("asd");
			$('#msg_send_ok').fadeIn(400).html("ثبت اطلاعات اولیه ملک با موفقیت صورت گرفت");
 ...
 if (loadId!=-1)
 {
  ...
  $.ajax({
     ...
     success: function(result){
      $("#city_loader").hide();
      $("#f3").html("<option value='-1'>شهر را انتخاب نمایید</option>");
      $("#f3").append(result);
     }
   });
  } 
