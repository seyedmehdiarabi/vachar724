...
<?php
$tbl_name="tbl_item_group";
$f_count=10;
$msg='';
$page_id=8;
$form_title="تعريف و مديريت گروه هاي کالايي";
		...
		<table border="0"  dir="rtl" cellspacing="0" style="border-collapse: collapse" class="main_table">
			...
			<?php print $form_title?></td>
				...
				<tr>
					...
					<?php 
					 if(isset($_GET["edit"])) {	?>				
					<img border="0" src="images/groups_pic/<?php print $selected_row[auto_id].'.jpg'?>" width="110">
					<?php }?>
...
<td align="right" width="20%" class="dirooz">عنوان سرگروه گروه کالا :</td>
<td align="right" width="35%" class="dirooz">
<select size="1" name="f10" dir="rtl">
<?php $re=mysql_query("select auto_id,f1 from tbl_item_main_group");
while($row2 = mysql_fetch_array($re))
{
?>
<option value=<?php print $row2[auto_id]; if(isset($_GET["edit"])) if ($selected_row["f10"]==$row2["auto_id"]) print " selected";?>><?php print $row2[f1] ?></option>
<?php }?>
