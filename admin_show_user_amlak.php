...
if($m_type=="1")
{
	$form_title="کلیه آگهی های املاک تایید شده";
	$str_where="tbl_amlak_master_info.f8=1 or tbl_amlak_master_info.f8=3";
}	
else
{
	$form_title="کلیه آگهی های املاک تایید نشده (در انتظار تایید ، رد شده و ...)";	
	$str_where="tbl_amlak_master_info.f8=0 or tbl_amlak_master_info.f8=2";
}	
access_page1($page_id,$_SESSION["s_user_type_id"]);

$str_sql="SELECT
  `tbl_item_sub_group`.`f2` as act_type, `tbl_amlak_master_info`.*
FROM
  `tbl_amlak_master_info` INNER JOIN
  `tbl_item_sub_group` ON `tbl_amlak_master_info`.`f1` =
    `tbl_item_sub_group`.`auto_id` where tbl_amlak_master_info.f2='$_SESSION[s_user_id]' and ($str_where) order by tbl_amlak_master_info.auto_id desc ";
...
<?php 
	...
	?>
										...
										<tr>
											...
											<td width="10%"><a href="admin_user_reg_amlak_pic.php?item_id=<?php print $row[auto_id]?>">ویرایش</a></td>
											<td width="10%"><a href="admin_user_reg_amlak_master.php?edit=1&item_id=<?php print $row[auto_id]?>">ویرایش</a></td>
											<td width="7%" align="center" class="<?php print $class_name; ?>"  >
	<?php print $row["adver_type"]?>						<td width="10%"><a target=_blank href="show_amlak_preview.php?item_id=<?php print $row[auto_id]?>">مشاهده</a></td>
