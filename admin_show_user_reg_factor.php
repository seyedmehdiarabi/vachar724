...
$form_title="";
$page_id=67;
$form_title="فاکتورهای پرداخت هزینه ثبت کالا توسط شما";
access_page1($page_id,$_SESSION["s_user_type_id"]);
$str_sql="SELECT auto_id,f2,f3 from tbl_item_reg_m_factor where f1='$_SESSION[s_user_id]' order by auto_id desc";
				...
				<div align="center" class="lazyload1">
					<table border="0" width="100%" cellspacing="0" cellpadding="0" style="border-collapse: collapse" id="tbl_kadr">
								<tr>
									...
									<?php print" فاکتور ثبت کالا شماره : <font color=#CC0066>". $row["auto_id"]."</font>" ?></td>
								</tr>
						<tr>
							<td>
...
$str_sql="SELECT  `tbl_payment`.`f4` AS `pay_date`, `tbl_payment`.`f5` AS `pay_time`,`tbl_payment`.`f6` AS `pay_method`,
		...
		FROM
		  `tbl_payment` where (f7=1 and f8='$row[auto_id]') and f1='$_SESSION[s_user_id]' order by auto_id";
